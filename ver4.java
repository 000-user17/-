...
class Algorithm {
    ...
    List<Service> services;//保存业务的信息，业务按照输入顺序依次加入
    Graph graph; //网络结构无向图

    int time; //时刻信息
    List<int[]> edgesPastStatus; //表示上一个时刻相关edge的一些情况, 保存{edge_idx, channel}即边的序号和其对应的channel信息

    int channelNum; //每一条边的通道数量
    long declineDis; //最大衰减距离
    int vertexNum; //顶点数量
    int edgeNumOri; //最初的边的数量
    int serviceNum; //一共有几个业务

    Map<String, List<Integer>> pathsMap = new HashMap<>();
    //构造图
...
//        System.out.println(str);
    ...
    public void forward(Service service, int outV, int inV, List<Integer> path, List<Integer> amplifiers){
    ...
    int cost; //总成本
    List<List<Integer>> allPaths;//保存所有的业务的通路
    List<List<Integer>> allAmplifiers;//保存所有业务的放大器

    //所有业务的动态前进状态
    //该算法对各个业务选择路径最短的路径作为其之后要行进的路线
    public void dynamicForward(){
