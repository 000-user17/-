...
public class ver3 {
    ...
    static Map<String, List<Integer>> pathsMap;
    //构造图
    public static void graphConstructAuto(List<String> example){

        time=0;
        String[] infos = example.get(0).split(" ");
        vertexNum = Integer.parseInt(infos[0]);
        int edgeNum = Integer.parseInt(infos[1]);
        edgeNumOri = edgeNum;//保存最初的边的数量，后续加边，新边编号就从edgeNumOri开始
        serviceNum = Integer.parseInt(infos[2]);
        channelNum = Integer.parseInt(infos[3]);
        declineDis = Long.parseLong(infos[4]);
        ...
        for(int i=0 ; i<edgeNum ; ++i){
            String[] edgeInfo = example.get(i+1).split(" ");
        ...
        for(int i=0 ; i<serviceNum ; ++i){
            String[] serviceInfo = example.get(i+edgeNum+1).split(" ");
    ...
    public static void dijkstra(int start, int end) {
        ...
        do {
            ...
            for (int i = 0; i < vertexNum; i++) {
                if (graph.edgeMatrix[minIndex][i] != 0 && graph.edgeMatrix[minIndex][i] < Long.MAX_VALUE) {
                    // 找到minIndex的所有邻接点
                    if (dist[minIndex] < Long.MAX_VALUE && graph.edgeMatrix[minIndex][i] + dist[minIndex] < dist[i]) {
                        // 新加入节点更短
    ...
    public static void forward(Service service, int outV, int inV, List<Integer> path, List<Integer> amplifiers){
        ...
        if(i==edges.size()){
            ...
            if(service.dist+edge.distance > declineDis){
                //增加放大器
                amplifiers.add(outV);
                service.dist = service.dist+edge.distance - declineDis;
            }else{
                service.dist = service.dist+edge.distance;
            }
    ...
    public static void dynamicForward(){
        ...
        while(!isEnd){
            ...
            for(Service service : services){
                ...
                forward(service, service.curLoc, nextV, allPaths.get(serviceIdx), allAmplifiers.get(serviceIdx));
...
//        int vertexNum = r.nextInt(3000)+1;
//        int edgeNum = r.nextInt(5000)+1;
//        int serviceNum = r.nextInt(10000)+1;
//        int channelNum = r.nextInt(80)+1;
//        int declineDis = r.nextInt(1000)+1;
        int vertexNum = 1000;
        int edgeNum = 5000;
        int serviceNum = 10000;
        int channelNum = 80;
        int declineDis = 1000;
    ...
    public static void main(String[] args){
        for(int i=0 ; i<1 ; ++i){
            while(true){
                List<String> exam = examGen();
                for(String info : exam){
                    System.out.println(info);
                }
                System.out.println("业务输出：");
                graphConstructAuto(exam);
